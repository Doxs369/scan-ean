...
// ============================================================
// RILEVAMENTO DATA SCADENZA - SCANSIONE CON TESSERACT.JS v5
// ============================================================
...
var maxOCRAttempts = 30;
var isOCRLoading = false;

/**
 * Inizializza il worker Tesseract.js v5
 * In v5 createWorker e async e accetta (lang, oem, options)
 */
function initExpiryOCR() {
  ...
  return new Promise(function(resolve, reject) {
    ...
    Tesseract.createWorker('eng', 1, {
      ...
      errorHandler: function(err) { console.error('Tesseract error:', err); }
    })
...
/**
 * Avvia la scansione della data di scadenza
 */
...
/**
 * Analizza frame con Tesseract.js v5
 */
function analyzeExpiryFrame() {
  ...
  if (statusDiv) statusDiv.textContent = 'Scansione... ' + ocrAttemptCount + '/' + maxOCRAttempts;
  ...
  // Ritaglia zona data (area centrale dove di solito e scritta la data)
  ...
  // OCR con Tesseract v5: worker.recognize(image, options, output)
